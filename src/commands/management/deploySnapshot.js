...
module.exports = {
    ...
    .setDescription(`Deploys a saved snapshot of the server.`)
    .addStringOption(opt => 
        opt.setName(`mode`)
        .setDescription(`The mode to apply the snapshot in.`)
        .setRequired(true)
        .addChoices(
            {name: `Clear - MOST DESTRUCTIVE OPTION`, value: `CLEAR`},
            {name: `Overwrite - Less destructive option`, value: `OVERWRITE`},
            {name: `Merge - Least destructive option`, value: `MERGE`},
        )
    )
    .addStringOption(opt => 
        opt.setName(`snapshot`)
        .setDescription(`The snapshot to deploy to the server.`)
        .setRequired(false)
    ),
    ...
    run: async ({ interaction }) => {
        ...
        let selected = interaction.options.getString(`snapshot`);
        ...
        try {
            ...
            const snapshot = await SnapshotData.findOne((selected)?{snapshot: selected}:{});
            if (!snapshot) {
                let embed = new EmbedBuilder()
                .setTitle(`❌ No Snapshot Detected`)
                .setDescription((selected)?`There is no snapshot with the timestamp of ${selected}. Please check which snapshot you are using.`:`There are no snapshots saved to deploy to. Please create a new snapshot before beginning deployment.`)
                .setColor(Colors.Red)
                .setTimestamp();
             ...
             * @param {string} step 
             */
            const updateWithCurrentStep = async (msg, embed, step) => {
                embed
            ...
            switch (option) {
                case 'CLEAR': {
                    ...
                    .setDescription(`You have selected the **clear** option. This command will delete all of the old channels, roles, and emojis first before applying the snapshot except the channel in which this command is being run in.\n\nAre you sure that you want to proceed with this command, knowing it could potentially be destructive?`)
                    .setColor(Colors.Red)
                    .setTimestamp();
                    ...
                    for (const [channelId, channel] of (await guild.channels.fetch())) {
                        ...
                        await updateWithCurrentStep(interaction, warningEmbed, `Deleting channel ${channel.name}`);
                        try {
                    ...
                    for (const [roleId, role] of (await guild.roles.fetch())) {
                        ...
                        await updateWithCurrentStep(interaction, warningEmbed, `Deleting role ${role.name}`);
                        try {
                    ...
                    for (const roleData of sortedRoles) {
                        await updateWithCurrentStep(interaction, warningEmbed, `Creating role ${roleData.name}`);
                        try {
                    ...
                    await updateWithCurrentStep(interaction, warningEmbed, `Sorting roles`);
                    try {
                    ...
                    for (const categoryData of categories) {
                        await updateWithCurrentStep(interaction, warningEmbed, `Creating category ${categoryData.name}`);
                        try {
                ...
                default: {
                    ...
                    .setDescription(`An invalid option was entered: ${option}`)
                    .setColor(Colors.Red)
                    .setTimestamp();
